...
class Autoencoder(pl.LightningModule):
    ...
    def on_train_start(self):
        ...
        batch = next(iter(self.trainer.train_dataloader))[0]
        print(f"Token shape: {len(batch)} {batch[0].shape}")
        print(f"Mask shape: {len(batch)} {batch[1].shape}")
        print(f"Position shape: {len(batch)} {batch[2].shape}")

        # Check if there are any overlapping files between train and val
        overlapping_files = set(self.trainer.train_dataloader.dataset.files).intersection(
            set(self.trainer.val_dataloaders.dataset.files)
        )
