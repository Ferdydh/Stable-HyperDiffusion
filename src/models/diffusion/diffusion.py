...
from models.utils import flattened_weights_to_inr
...
class INRDiffusion(pl.LightningModule):
    ...
    def visualize_samples(self, prefix: str, batch_idx: int):
        ...
        for i, sample in enumerate(samples):
            ...
            fig = plot_image(
                flattened_weights_to_inr(sample, self.demo_inr), self.device
            )
        ...
        if self.fixed_samples is not None:
            for i, fixed in enumerate(self.fixed_samples):
                fig = plot_image(
                    flattened_weights_to_inr(fixed, self.demo_inr), self.device
                )
