...
class INRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        tokens, masks, pos = weights_to_tokens(
            state_dict, tokensize=2, device=self.device
        )
...
class DataHandler(pl.LightningDataModule):
    def __init__(
        ...
        self.is_for_mlp = isinstance(config, MLPExperimentConfig) or isinstance(config, DiffusionExperimentConfig)
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            ...
            output_train = os.path.join("data", self.config.logging.run_name + "_train.txt")
            output_val = os.path.join("data", self.config.logging.run_name + "_val.txt")
