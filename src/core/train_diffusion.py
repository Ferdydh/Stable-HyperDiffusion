...
import atexit
from src.core.config import DiffusionExperimentConfig, TransformerExperimentConfig, TransformerConfig
...
from src.models.diffusion.hyperdiffusion import HyperDiffusion
...
def train(
    ...
    try:
        ...
        if config.logging.run_name is None:
            ...
            config.logging.run_name = f"{config.logging.run_name} {datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        model = Transformer(
            layers, layer_names, **(TransformerConfig.get_dict(config.transformer_config))
        ).cuda()
